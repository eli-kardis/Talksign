      ...
      try {
        ...
        if (error) {
          ...
          if (error.message?.includes('network') || error.message?.includes('timeout')) {
            ...
            // ✅ refresh token 에러 시 localStorage 지우지 않고 세션만 종료
            console.warn('Invalid refresh token detected, clearing session only (not localStorage)')
            try {
              // 세션만 종료 (localStorage는 보존)
