...
export function Login({ onNavigate }: LoginProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      ...
      // username 추출 (이메일의 @ 앞부분)
      const username = session.user.email?.split('@')[0] || 'dashboard'
      console.log('[Login] Email login successful, redirecting to dashboard')
