...
export default function UserPaymentsPage({ params }: { params: Promise<{ username: string }> }) {
  ...
  useEffect(() => {
    if (!isLoading && !user) {
      console.log('Payments: User not authenticated, redirecting to signin')
  ...
  useEffect(() => {
    if (!isLoading && user) {
      ...
      if (username !== userUsername) {
        router.replace(getUserPath(userUsername, '/finance/payments'))
      }
    }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">결제 로딩 중...</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <FinanceView
      ...
      onTabChange={(tab) => {
        ...
        if (tab === "tax-invoices") {
          router.push(getUserPath(userUsername, '/finance/tax-invoices'))
        }
      }}
      onNavigate={(view) => {
        const userUsername = user?.email ? extractUsername(user.email) : username
        ...
        router.push(map[view] ?? getUserPath(userUsername, '/finance/payments'))
      }}
    />
  )
}
