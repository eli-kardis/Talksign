...
export default function UserContractsPage({ params }: { params: Promise<{ username: string }> }) {
  ...
  useEffect(() => {
    if (!isLoading && !user) {
      console.log('Contracts: User not authenticated, redirecting to signin')
  ...
  useEffect(() => {
    if (!isLoading && user) {
      ...
      if (username !== userUsername) {
        router.replace(getUserPath(userUsername, '/documents/contracts'))
      }
    }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">계약서 로딩 중...</p>
        </div>
      </div>
    )
  }
