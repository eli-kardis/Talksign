...
export default function UserCustomersPage({ params }: { params: Promise<{ username: string }> }) {
  ...
  useEffect(() => {
    if (!isLoading && !user) {
      console.log('Customers: User not authenticated, redirecting to signin')
  ...
  useEffect(() => {
    if (!isLoading && user) {
      ...
      if (username !== userUsername) {
        router.replace(`/${userUsername}/customers`)
      }
    }
